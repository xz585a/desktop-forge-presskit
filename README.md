...
- スクリーンショット **13場面 × 日英 = 26枚**（1920×1080、全て無加工の PNG）
  - 言語切り替えに連動し、日本語表示なら日本語UI、英語表示なら英語UIの画が出る
  - **09 世界図 / 10 受注 / 11 国の詳細は 2026-09-06 追加。**世界MAP導入分
- ループ動画 **8場面 = 13ファイル**（mp4）
  - 鑑定・個性・世界図・冒険者の出撃・冒険ログの5場面は、`_ja` を付けた日本語UI版も置いてあり、言語切り替えに連動する
  - 全体像・工房作業・感情アイコンの3場面は文字が出ないため、日英で同じファイルを使う
  - **`loop_world_map` は 2026-09-06 追加。**ストア説明文のエクストラ画像 `map` と同じ素材で、
    ページ表示用に音声を落として CRF 23 で再エンコードしてある（原寸は `release/media/store_description/`）
- トレーラー **日英2本**（mp4、各108秒。字幕だけが異なり、映像内のゲーム UI は両方とも英語）
  - **2026-09-06 に世界パート入りの v04 へ差し替え済み。**
  - `DesktopForge_Trailer.mp4` が英語字幕、`DesktopForge_Trailer_ja.mp4` が日本語字幕
- ロゴ（透過 PNG）とカプセル画像6点（`assets/branding/`。Steam へ出したものと同じ原寸）
- ファクトシート 日英
- 一括ダウンロード用 zip（62MB。トレーラーは容量の都合で含めない）
...
- 日本語字幕版: <https://youtu.be/k6giLq7QAT8>
- 英語字幕版: <https://youtu.be/clRU4E9Q09Y>

**2026-09-06 に v04（世界パート入り・108秒）へ差し替えた。**埋め込み・Releases の mp4・
ファクトシートの3箇所とも同じ v04 を指している。

**旧 v03（`lH1CPiquSWI` / `MmC9m-qBp0Q`）は削除しない。**ファミ通.com と gamebiz の記事から
導線が通っており、非公開にすると記事側の埋め込みが死ぬ。限定公開のまま残してある。

トレーラーを作り直したら**新規アップロードになり、ID は必ず変わる。**上の2箇所と
`index.html` の `iframe`、`fact_sheet_*.txt` を同時に直すこと。
